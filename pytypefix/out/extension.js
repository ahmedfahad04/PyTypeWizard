...
async function installPyre() {
    ...
    if (!pipPath) {
        const choice = await vscode.window.showErrorMessage('pip3 or pip not found. Please install pip in your environment.', 'Show me how', 'Use Terminal', 'Cancel');
...
async function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('extension.runPyreCheck', async () => {
        ...
        if (!installed) {
            const shouldInstall = await vscode.window.showInformationMessage('Pyre is not installed or not in PATH. Would you like to install it?', 'Yes', 'No', 'Use Terminal');
